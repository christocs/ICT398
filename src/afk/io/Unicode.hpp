...
namespace afk {
  namespace io {
    /**
     * Returns the underlying C string in a std::u8string.
     *
     * @param s The u8string.
     * @return The underlying C string.
     */
    auto to_cstr(const std::u8string &s) -> const char *;

    /**
     * Converts a UTF-8 C string to a C string.
     *
     * @param s The UTF-8 C string.
     * @return The resulting C string.
     */
    auto to_cstr(const char8_t *s) -> const char *;

    /**
     * List of the Unicode character ranges.
     */
    // clang-format off
    constexpr auto unicode_ranges = utility::array_of<ImWchar>(
